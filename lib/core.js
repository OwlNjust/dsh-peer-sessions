...
import { listAddressable, listWithHidden, resolveTarget, formatCandidate, asRecord } from './addressable.js'
...
export class PeerCore {
   ...
   */
  constructor(ctx, store = new PeerStore()) {
    this.ctx = ctx
    this.store = store
  }
  ...
  progress(peerEntry) {
    ...
    if (keys.length > 0) {
      ...
      if (goalSnapshot !== undefined && typeof goalSnapshot.objective === 'string') {
        const phase = typeof goalSnapshot.phase === 'string' ? ` [${goalSnapshot.phase}]` : ''
        const rounds = typeof goalProjection.roundsStarted === 'number' ? ` · round ${goalProjection.roundsStarted}` : ''
        lines.push(`  goal: ${goalSnapshot.objective}${phase}${rounds}`)
      }
      ...
      if (Array.isArray(values.todos)) {
        ...
        const now = typeof active?.content === 'string' ? ` · now: ${clip(active.content)}` : ''
        lines.push(`  todos: ${done} of ${todos.length} complete${now}`)
      }
      ...
      if (Array.isArray(values.turnOutline) && values.turnOutline.length > 0) {
        lines.push(`  turns so far: ${values.turnOutline.length}`)
        for (const raw of values.turnOutline.slice(-RECENT_TURNS)) {
          const entry = asRecord(raw)
          if (entry === undefined) continue
          const prompt = typeof entry.prompt === 'string' && entry.prompt !== '' ? clip(entry.prompt) : '(no preview)'
          const response = typeof entry.response === 'string' && entry.response !== '' ? ` → ${clip(entry.response)}` : ''
          lines.push(`    turn ${String(entry.turn)}: ${prompt}${response}`)
        }
      }
