...
import { listAddressable, listWithHidden, resolveTarget, asRecord } from './addressable.js'
...
export class PeerCore {
  ...
  async resolveOrRefuse(query, selfId, signal) {
    ...
    const lines = this.candidateLines(found.candidates)
    if (found.kind === 'many') {
      throw new PeerRefusal(this.t('resolve.many', { query, count: found.candidates.length, lines }))
    }
    if (found.candidates.length === 0) throw new PeerRefusal(this.t('resolve.noneAlone'))
    throw new PeerRefusal(this.t('resolve.noneWithCandidates', { query, lines }))
  }
  ...
  async deliver({ selfAgent, selfLabel, peerEntry, payload, silent = false, signal }) {
    ...
    if (channel === undefined) {
      ...
      // running peer: waking one would contradict the request outright. Report
      // that rather than quietly doing the opposite of what was asked.
      if (willWake && silent) return { outcome: 'silent-needs-running', peer }
    ...
    // Waking needs no separate consent: the channel's approval covers it, and a
    // card that stated it was shown when the channel was opened.
    const target = live ?? (await this.wake(peer))
    if (target === undefined) {
    ...
    // Remember the request so its answer can ride this same grant.
    ...
    return { outcome: 'delivered', peer, channel, item, silent, ridesExistingGrant }
  }
  ...
  progress(peerEntry) {
    ...
    lines.push(t('progress.lastActive', { iso: new Date(peerEntry.updatedAt).toISOString() }))
