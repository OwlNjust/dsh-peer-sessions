...
export class PeerCore {
  ...
  progress(peerEntry) {
    ...
    if (keys.length > 0) {
      ...
      // Every projection value is JsonValue, so `null` is a legal value and
      // `!== undefined` alone would read a property off it.
      const goal = asRecord(values.goal)
      ...
      const todo = asRecord(values.todo)
      if (todo !== undefined && Array.isArray(todo.items)) {
        const done = todo.items.filter((item) => asRecord(item)?.status === 'completed').length
