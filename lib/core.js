...
export class PeerCore {
  ...
  async deliver({ selfAgent, selfLabel, peerEntry, payload, silent = false, signal }) {
    ...
    // conversations. Both are checked BEFORE the budget is spent — a refused
    // delivery must not consume the channel's quota.
    if (!this.store.withinRate(channel)) {
      throw new PeerRefusal(this.t('rate.limited', { limit: RATE_LIMIT, seconds: RATE_WINDOW_MS / 1000 }))
    }
    const hop = this.store.nextHop(selfId)
    if (hop > HOP_LIMIT) {
    ...
    this.store.noteInboundHop(peerId, hop)
  ...
  noteReply(sessionId, requestId, channel) {
    ...
    // An answer retires the request's deadline, whether or not a matching inbox
    // item was found: the waiting is over either way.
