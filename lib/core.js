...
export class PeerCore {
  ...
  async deliver({ selfAgent, selfLabel, peerEntry, payload, silent = false, signal }) {
    ...
    if (channel === undefined) {
      const tier = await askGrant(this.ctx, { agent: selfAgent, peerLabel: peer.label, selfLabel, signal, t: this.t })
  ...
  async ensureLive(peerEntry, selfAgent, signal) {
    ...
    const approved = await askWake(this.ctx, { agent: selfAgent, peerLabel: peerEntry.label, signal, t: this.t })
