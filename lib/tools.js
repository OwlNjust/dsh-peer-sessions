...
import { readMarker, stateLabel, HOP_LIMIT } from './store.js'
 ...
 *
 * A hop refusal carries an extra paragraph, because the bare refusal was
 * actively misleading: "peer messages may not relay onward indefinitely" reads
 * as "try another way", so a live user asked for more quota and retried, and
 * both sides burned a turn discovering that no sending pattern helps. The
 * refusal says what it counts, what cannot reset it, and what actually works.
 */
function messageOf(error, t) {
  if (error instanceof PeerRefusal) {
    // The hop refusal is the one refusal that needs a paragraph, and the
    // paragraph must be in the caller's language — an English explainer after a
    // Chinese refusal is its own kind of broken.
    const key = /hop|跳/.test(error.message) ? 'hop.help' : undefined
    const help = key === undefined || t === undefined ? '' : t(key)
    return help === '' ? error.message : `${error.message}\n\n${help}`
  }
    ...
    defineTool({
      ...
      parameters: {
        ...
        summary: {
          type: 'string',
          required: true,
          // Required for EVERY kind, which the old description left unsaid: a
          // caller omitted it on a notice, the host's schema validation refused
          // the call, and the plugin never ran.
          description: 'Required. One self-contained line the peer can act on without asking anything back.',
        },
      ...
      async execute(args, exec) {
        ...
        try {
          ...
          return [
            ...
            typeof result.hop === 'number' ? `  hop:        ${result.hop}` : undefined,
            `  ${queueNote}`,
            // B5 rides here: an overdue request is announced on the next thing
            // this conversation does, announced once, and never resent.
            ...core.pendingReports(agent.id),
          ]
            .filter((line) => line !== undefined)
            .join('\n')
        } catch (error) {
          return messageOf(error, core.t)
        }
      },
    }),
  )
    ...
    defineTool({
      ...
      async execute(args, exec) {
        ...
        if (typeof args.id === 'string' && args.id !== '') {
          ...
          // The deadline is its OWN axis, not part of `state:` — whether a body
          // was read, whether a request was answered, and whether its deadline
          // passed are three independent facts. The listing shows all three, and
          // a peer that had just fetched the body rightly objected that the one
          // place it was missing was the header — exactly where a reader looks to
          // ask "did this expire?".
          if (core.requestOverdue(item.requestId)) lines.push('deadline: OVERDUE')
          lines.push('', item.text)
          return lines.join('\n')
        }

        // Overdue requests are collected FIRST, before any early return. The
        // notice about a request this conversation is WAITING on has nothing to
        // do with whether anything arrived here — and reporting it consumes it,
        // so an early return that skipped this silently swallowed the one
        // announcement B5 promises. Found on the live harness, where an empty
        // inbox answered "Peer inbox is empty." and the notice never appeared.
        const overdue = core.pendingReports(selfId)
        const items = core.store.inboxFor(selfId)
        if (items.length === 0) {
          return ['Peer inbox is empty.', ...overdue].join('\n')
        }
        ...
        for (const item of items) {
          ...
          // B6 asks the inbox to show "timed out" as its own thing. An overdue
          // request is still unanswered, so it keeps that fact — the deadline
          // marker is added rather than replacing it.
          const marker = core.requestOverdue(item.requestId)
            ? `timed out · ${readMarker(item.status)}`
            : readMarker(item.status)
          lines.push(
        ...
        // An unanswered request the reader is WAITING on is reported here too:
        // reading the inbox is exactly when someone would wonder about it.
        if (overdue.length > 0) lines.push(...overdue)
        return lines.join('\n')
      },
    }),
  )
      ...
      async execute(_args, exec) {
        ...
        if (channels.length === 0) {
          ...
          for (const channel of channels) {
            ...
            // Two independent axes, and a live report was locked out by the one
            // that was invisible: `remaining` is the MESSAGE quota, while the
            // hop ceiling bounds how deep a RELAY chain may go. Showing only the
            // first made a hop refusal look like a quota problem, so both are
            // printed and named.
            const hop = core.store.nextHop(selfId, peerId)
            const hopsLeft = Math.max(0, HOP_LIMIT - hop)
            lines.push(
              `- "${peer?.label ?? peerId}" · tier=${channel.tier} · remaining=${channel.remaining ?? 'unlimited'} (messages)` +
                ` · hop=${hop}/${HOP_LIMIT} (relay depth, ${hopsLeft} left) · peer=${state}`,
            )
          }
        }
        ...
        // What this conversation is still owed: the other half of B5, so nobody
        // has to poll a peer to find out whether an answer is coming.
        //
        // `pendingReports` is deliberately NOT consumed here. This listing is a
        // read-only view and its `OVERDUE` marker already states the same fact;
        // announcing it here too would say it twice, once per surface. The one
        // announcement rides the next thing that actually DOES something
        // (`peer_send` / `peer_inbox`).
        const owed = core.store.pendingFor(selfId)
        if (owed.length > 0) {
          lines.push('', `Waiting for ${owed.length} answer(s):`)
          for (const record of owed) {
            const due = record.dueAt === Number.POSITIVE_INFINITY ? 'no deadline' : `due ${new Date(record.dueAt).toISOString()}`
            const late = core.store.isOverdue(record.requestId) ? ' · OVERDUE' : ''
            lines.push(`- ${record.requestId} -> "${record.toLabel ?? record.to}" · ${due}${late}`)
          }
        }
        return lines.join('\n')
      },
