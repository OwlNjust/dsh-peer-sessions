      ...
      async execute(args, exec) {
        ...
        try {
          ...
          if (result.outcome === 'target-not-running') {
            return `"${peer.label}" is not running and the user did not approve waking it. Nothing was sent. Mention it once, then move on.`
          }

          if (args.kind === 'reply' && args.replyTo !== undefined) core.noteReply(agent.id, args.replyTo)
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        ...
        'and requests whose deadline has passed.',
      parameters: {},
      output: textOutput,
      async execute(_args, exec) {
        const guard = callerGuard(core, exec)
        if (guard.refused !== undefined) return guard.refused
        const items = core.store.inboxFor(guard.agent.id)
        ...
        for (const item of items) {
          ...
          lines.push(
            `- [${item.status}] ${item.kind} from "${item.fromLabel}" (${item.from}) · ${age} min ago`,
            `  id: ${item.id}${item.requestId === undefined ? '' : ` · request: ${item.requestId}`}`,
            `  ${item.summary}`,
          )
        }
        return lines.join('\n')
