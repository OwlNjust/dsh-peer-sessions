...
/** How many request issuers to remember, for the reply direction check. */
const REQUEST_MEMORY = 500

/**
 * How long an unanswered request keeps being reported before it is forgotten.
 *
 * Without this, `pending` only ever grew: `clearPending` fires on a matching
 * reply, so a request that was revoked, declined or simply ignored stayed in the
 * map forever — one entry per unanswered request, scanned on every inbox call,
 * and `peer_list` kept announcing "Waiting for N answers" for channels that had
 * been revoked long ago.
 */
const PENDING_GRACE_MS = 24 * 60 * 60_000
...
export class PeerStore {
  constructor() {
    ...
    /** Channel counter. NOT a message counter — ids must not borrow it. */
    this.seq = 0
    /**
     * Monotonic message counter, separate from {@link seq} on purpose.
     *
     * Message ids used to be built from the CHANNEL counter plus millisecond
     * time, so two deliveries landing in the same millisecond produced the same
     * id: `peer_inbox(id)` then returned the wrong body (the first match wins)
     * while the other item became unreachable by id. Both auditors found this
     * independently. A per-store counter cannot collide.
     */
    this.messageSeq = 0
    /**
     * `requestId -> issuing session id`, kept independently of `pending` and of
     * any single channel. `pending` is cleared when a request is answered, and a
     * channel can be revoked and reopened, so neither can be trusted to answer
     * "who issued this request?" later — which the direction check needs.
     * Bounded by {@link REQUEST_MEMORY}, oldest evicted first.
     * @type {Map<string, string>}
     */
    this.requestIssuers = new Map()
  }
  ...
  /**
   * Remove one channel, along with the deadlines it was holding.
   *
   * A revoked channel cannot carry the answer any more, so leaving its pending
   * records behind makes `peer_list` promise an answer that can never come.
   * `requestIssuers` is deliberately NOT pruned here: it is what the reply
   * direction check reads, and a `once` exchange revokes its channel as it
   * spends it, so dropping issuers would disarm that guard immediately.
   */
  revoke(channel) {
    this.channels.delete(pairKey(channel.a, channel.b))
    for (const [requestId, record] of this.pending) {
      if (record.channelId === channel.id) this.pending.delete(requestId)
    }
  }
  ...
  /**
   * Record that `sessionId` received a peer message: how deep it was, and who
   * sent it.
   * @param sessionId - the receiving session.
   * @param hop - the hop count carried by the message it received.
   * @param from - the session that sent it.
   */
  noteInboundHop(sessionId, hop, from) {
    this.inboundHops.set(sessionId, { hop, from, at: Date.now() })
  }

  /**
   * The hop count a message sent right now should carry.
   *
   * HOPS COUNT RELAYS, NOT REPLIES. Answering the conversation that just wrote
   * to you keeps the same hop; handing the message to a DIFFERENT conversation
   * — relaying it onward — adds one.
   *
   * The first version incremented on every send while a recent inbound message
   * existed, so four ordinary back-and-forth exchanges between two peers walked
   * 1,2,3,4 and hit the ceiling permanently: a live report described three
   * collaborations locked out mid-task, with no way back and `peer_list` showing
   * healthy quotas that described a different axis entirely. Ordinary peer
   * conversation must not be what the guard counts; only propagation across
   * conversations is.
   *
   * @param sessionId - the sending session.
   * @param toId - the session it is about to send to.
   * @returns the hop count to carry.
   */
  nextHop(sessionId, toId) {
    ...
    // this is a new chain, not a reply to something from twenty minutes ago.
    if (Date.now() - mark.at > HOP_MEMORY_MS) {
      this.inboundHops.delete(sessionId)
      return 1
    }
    // Replying to whoever wrote to us continues the same chain.
    if (toId !== undefined && mark.from === toId) return mark.hop
    return mark.hop + 1
  }
   ...
   */
  notePending(record) {
    this.pending.set(record.requestId, { ...record, reported: false, notedAt: Date.now() })
  }
  ...
  /**
   * Mint the next inbox message id.
   *
   * Store-scoped and monotonic. It must NOT reuse {@link seq} (the channel
   * counter) plus a timestamp: that collides for two deliveries in the same
   * millisecond, and a colliding id makes `peer_inbox(id)` return the wrong
   * body while the other item becomes unreachable by id.
   * @returns a unique message id.
   */
  nextMessageId() {
    this.messageSeq += 1
    return `pm-${this.messageSeq}-${Date.now().toString(36)}`
  }

  /**
   * Who issued a request, as far as the store can tell: from its pending record
   * (authoritative while it is unanswered) or from the channel that carried it.
   * @param requestId - the request in question.
   * @param channel - the channel it travelled on, when known.
   * @returns the issuing session id, or undefined when unknown.
   */
  issuerOf(requestId, channel) {
    const remembered = this.requestIssuers.get(requestId)
    if (remembered !== undefined) return remembered
    const record = this.pending.get(requestId)
    if (record !== undefined) return record.from
    if (channel !== undefined) return channel.openRequests.get(requestId)
    return undefined
  }

  /**
   * Remember which session issued a request, so a later reply naming it can be
   * checked for direction even after the pending record is gone.
   * @param requestId - the request.
   * @param from - the session that issued it.
   */
  noteIssuer(requestId, from) {
    this.requestIssuers.set(requestId, from)
    while (this.requestIssuers.size > REQUEST_MEMORY) {
      const oldest = this.requestIssuers.keys().next().value
      this.requestIssuers.delete(oldest)
    }
  }

  /**
   * Drop requests nobody ever answered, once their grace window has passed.
   * Called from the readers below, so a long-lived process cannot accumulate
   * one entry per abandoned request.
   * @param now - current time.
   */
  forgetStale(now = Date.now()) {
    for (const [requestId, record] of this.pending) {
      if (record.notedAt !== undefined && now - record.notedAt > PENDING_GRACE_MS) {
   ...
   *
   * Deliberately kept with no caller: the audit of v1.0.0 confirmed it is dead
   * code, and it stays because "this conversation is gone" is the one event that
   * should clear all of its state, and the next caller will reach for a named
   * method rather than hand-rolling the loop. Note what it does NOT clear:
   * `requestIssuers`, which the reply direction check reads.
   */
   ...
   *
   * Also kept deliberately (audit of v1.0.0 found no caller). It is the natural
   * cleanup to run when the sidebar set is re-read and a pair has gone
   * unreachable, and `revoke` now clears each dropped channel's pending records.
   */
