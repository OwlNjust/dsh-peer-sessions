...
/**
 * Why one session is out of reach, as a stable code.
 *
 * The code — not the English text below — is what callers branch on: the text
 * is kept only for logs, and every human-facing explanation is localized from
 * the code at the call site.
 */
export const HIDDEN_ARCHIVED = 'archived'
export const HIDDEN_SUBAGENT = 'subagent'
export const HIDDEN_BLANK = 'blank'

const REASON_TEXT = {
  [HIDDEN_SUBAGENT]: 'it is a subagent session',
  [HIDDEN_ARCHIVED]: 'it is archived',
  [HIDDEN_BLANK]: 'it is a blank placeholder, not a conversation',
}
 ...
 * @returns whether the session is addressable, and its reason code when it is not.
 */
export function classify(summary, archived) {
  if (summary.origin === 'subagent') {
    return { addressable: false, code: HIDDEN_SUBAGENT, reason: REASON_TEXT[HIDDEN_SUBAGENT] }
  }
  if (archived.has(summary.sessionId)) {
    return { addressable: false, code: HIDDEN_ARCHIVED, reason: REASON_TEXT[HIDDEN_ARCHIVED] }
  }
  if (summary.blank === true) {
    return { addressable: false, code: HIDDEN_BLANK, reason: REASON_TEXT[HIDDEN_BLANK] }
  }
...
export function labelOf(ctx, summary) {
  ...
  // Guarded, not assumed: `cwd` is typed optional-string, but a label built from
  // it also lands in delivered message headers and in the plugin-sourced audit
  // notice, so a null or non-string must degrade to the id rather than throw a
  // TypeError that the tool layer would then report as if it were an answer.
  if (typeof summary.cwd === 'string') {
...
export function stripInvisible(text) {
  // The class covers what a paste actually carries, not just the canonical
  // zero-width space: bidi ISOLATES (U+2066-U+2069) and the deprecated
  // U+2065-U+206F block, soft hyphen, Arabic letter mark, Mongolian vowel
  // separator, and variation selectors. The first version stopped at U+2064 and
  // so let the very failure this function exists to prevent happen again —
  // measured, six common characters still got through. Keep it a superset of
  // the host's own sanitizer (`dsh-session-title`).
