...
const MESSAGES = {
  en: {
    ...
    'channel.revoked':
      'That channel was revoked while the approval was pending, so nothing was sent. Open a new one if you still ' +
      'want to send this.',
    'rate.limited':
      'This pair has hit its rate limit ({limit} deliveries per {seconds}s). Nothing was sent. Wait, or batch ' +
      'several messages into one.',
    'hop.exceeded':
      'Refused: this message would be hop {hop} of a relay chain capped at {limit}. Nothing was sent.',
    'hop.help':
      'This ceiling bounds how deep a RELAY chain may go: a message passed from one conversation to a third, then ' +
      'a fourth. Replying to the conversation that wrote to you does NOT count, so ordinary back-and-forth is ' +
      'never what is refused here. Nothing resets it: not a new message, not a different kind, and not more user ' +
      'authorization — the channel already carries the user\'s consent. If the message must still get there, ask ' +
      'the user to relay it, or use files if the conversations share a workspace.',
  ...
  zh: {
    ...
    'channel.revoked':
      '在你批准期间这条通道已被撤销，所以没有发送任何内容。如果仍要发送，请重新建立通道。',
    'rate.limited':
      '这一对会话已达速率上限（每 {seconds} 秒 {limit} 条）。没有发送任何内容。请稍等，或者把多条合并成一条。',
    'hop.exceeded':
      '已拒绝：这条消息会成为接力链条的第 {hop} 跳，而上限是 {limit} 跳。没有发送任何内容。',
    'hop.help':
      '这条上限管的是**接力深度**：一条消息被从 A 会话转给 C，再转给 D。**回复那个刚给你写信的会话不算接力**，' +
      '所以正常的来回对话永远不会因为它被拒。它也没有任何重置手段：换消息类型不行、新开一条消息不行、' +
      '再多要一次用户授权也不行（通道里已经有用户的同意）。如果这条消息必须送到，请让用户代为转达；' +
      '若两个会话共享工作区，改用文件交换。',
...
function fill(template, params) {
  // `null` must behave like "no parameters": reading a property off it threw a
  // TypeError from deep inside the string layer, which the tool catch-all would
  // then report to the model as an answer.
