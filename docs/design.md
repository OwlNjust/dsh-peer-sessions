...
8. **`once` 档与「请求-回复」回路不匹配。**
   实测：批准「仅本次」后 `remaining=0`，通道随即作废。于是**对端的回复需要一次新的批准**，而且那张卡片出现在**对端会话**里——用户会看到"我不是刚批准过吗"。
   语义上"仅本次投递"确实是这个意思，所以这是一个**待定的产品决策**，不是实现缺陷。可选方向见下。

### 仍未在生产中验证的部分

- 对端**回复**路径：`peer_inbox` 记到待回复项、`peer_send(kind:'reply')` 正确销账。只读路径与**发送**路径（含冷会话唤醒、来源标注、发送方审计）均已在真机验证通过。
