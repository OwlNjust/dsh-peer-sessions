...
| **接力的跳数实测**（v1.0.1 之后） | 本端主动起头 `hop: 1`；对端**回复**时仍是 `hop: 1`（同一条对话的延续）；本端再回复仍是 `1`。只有把消息交给**第三方**会话时才变成 `2`，再往下接力 `3`，第 4 跳被拒。回归测试 `replying to the other side does not deepen the chain` 用**六次往返**钉住了这一点 |
