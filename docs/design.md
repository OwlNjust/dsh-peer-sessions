...
| **取回抬头缺 `timed out`**（对端指出，已修） | 列表显示 `[timed out · …]`，而同一条的取回抬头里**完全没有超时痕迹**——恰好在最需要它的地方没有：人刚取回正文，正是会问"这条过期了吗"的时刻。修法是给期限**单独一行** `deadline: OVERDUE`，而**不塞进 `state:`**（那是第三个轴，塞进去会把三件事混成一件）——这一点由提出者本人指出 |
