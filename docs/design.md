...
**真机事实（改变了本节的设计）**：在这个部署里**空闲会话就是冷的**。一次真机观察里，CodeTools 被唤醒、跑完一轮、回了信，随后又变回 `running: false`；`peer_list` 列出的 17 个会话全是 `not running`。
...
| **归档即时拒绝** | 归档 CodeTools 后它**立刻从列表中消失**（17 → 16），按名字投递给它被拒绝、未发送任何内容。这条同时证明 H1.1（投递前重读）与 H1.2（不缓存） |
