...
- **B2** 投递只走 `followup()`，**绝不用 `steer()`** → 天然实现"空闲才投递"。
  ...
  **已接受并刻意保留的后果（真机观察后确认）**：`followup` 的契约是"排一个独立回合并唤醒驱动"，所以入站的对端消息会**以排队用户回合的形式出现在客户端的输入区**，可被编辑、删除、重排——看起来像收件人自己待发送的内容。
  ...
  - **一次未复现的观察，记录以免将来误查本插件**：对端消息停在 composer 队列里时，曾出现"选中斜杠命令后 Backspace 删不掉已输入内容"。同一次里 `/goal`（内置命令）表现**完全相同**，随后两个命令又都恢复正常，因此**与本插件无关、也未能确认成因**。若再次出现，先记录当时 composer 里是否挂着排队消息。
...
- **撤销**：`/peer revoke` 只有人能手打，需要你执行一次。
- **归档即时拒绝**：投递时重读侧栏列表（H1.2），单测覆盖；真机验证需要归档一个会话。
- `peer_inbox` 的**已读推进**：目前没有把条目标为已读的入口，回复会一直显示 `[unread]`（属 M2）。
