...
| **跳数上限** | `HOP_LIMIT = 3` | `store.nextHop` / `noteInboundHop` | 一条**因果链**跨会话传播的深度 |
...
**跳数为什么由插件记、而不是写进消息让模型带**：模型看不到也不需要维护这个数字。若靠模型在 `peer_send` 里传递，它漏传一次链条就断，而**断掉的是防护**。做法是收发的两侧都由 store 记：投递时 `noteInboundHop(接收方, hop)`，发送时 `nextHop(发送方)` = 收到的 hop + 1。没有入站记录（或记录已过期）时按 **1** 起算——那是"用户自己起的话头"，不是别人的回音。
...
| **跳数在真机上的递进** | 本端主动起头 `hop: 1`；对端在回答那条 hop 1 的请求时发出 `hop: 2`。链条按设计逐跳加深 |
...
| **跳数跨会话实测**（同一链条两端的值） | 一条链上：本端主动起头 `hop: 1`；对端收到它、回答时发出 `hop: 2`；该 hop 2 的消息进入本端收件箱后，本端再回复即会是 3。对端还专门问了"你预测我这边是 1 还是 2"——**实测 2**，两边规则一致（`nextHop = 收到的 hop + 1`），只是各自站在链条的不同位置 |
