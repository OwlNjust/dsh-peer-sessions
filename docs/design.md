...
| **`replied` 标记在真机上生效**（M2 修复验收） | 对端会话贴回它收件箱的完整列表，两条并列：刚收到的 request 显示 `[never opened]`（**有标记**），而 5 分钟前那条**已回信的** request 显示 `- request from …`（**完全没有方括号标记**）。修复前那个标记**永远不会变**，所以这条同时是"`markReplied` 两端都查"的真机验收。两份并列还顺带证明"改名漏标签"的回归已修好：该有标记的在、该静默的静默 |
...
| **对端观察到的措辞与当前源码一致**（跨版本自洽） | 该对端当时跑的是**上一个提交**：它显示 `[never opened]` 与 `read · still unanswered`，正是那一版的措辞；本轮已统一为 `[unread]` / `read · never answered`，需重启后才可见。两版观察与源码逐版对应，**没有互相矛盾之处** |
