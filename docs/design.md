...
- **`peer_inbox` 只能给摘要，取不回正文**。真机暴露的：对端回了一封**结构完整、很长**的答复，而 `peer_inbox` 只显示 `summary`（一行），正文只能从转录里那条 relay 消息读到。于是"收件箱"其实只是提示而非索引——**那条消息若不在当前上下文里，正文就取不回来**。应让 inbox 能按 id 取回正文。
- `peer_inbox` 摘要里补上 `replyTo`（正文里有，摘要里漏了）
- `peer_progress` 深化：`plan`、`contextPressure` 等投影的渲染
- 环路防护完整版：每对速率上限、跳数上限、请求超时后的主动通知
- `peer_inbox` 的已读推进（目前没有入口把条目标为已读）
